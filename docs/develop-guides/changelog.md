...
- 普通用户可管理自己的知识库：`knowledge_router` 不再一律 `get_admin_user`，列表/创建/类型/分块对登录用户开放，按 kb 读写改为 `get_kb_reader` / `get_kb_manager`（创建者可管，共享库只读，非可访问 403）。非管理员创建强制私有，并受 `KB_MAX_PER_USER`（默认 3）配额限制。评估/stats 仍限管理员。前端知识库入口对所有人开放，详情页按 `can_manage` 隐藏编辑/上传，评估 tab 仍只给管理员。
- 优化智能体对话为 ChatGPT 式连续阅读与操作体验：用户消息改用中性气泡并增加编辑重发、复制操作，助手消息统一展示模型、赞踩、复制、重新生成与来源入口，长对话离开底部后提供回到底部按钮，保留原有流式停止、自动增高输入、附件和 @ 提及能力；基础 Agent Prompt 不再写死“语析”身份，而是注入本次运行真实 `model_spec` 及实际解析出的工具、知识库、Skills 和子智能体，询问模型或能力时必须据此如实回答，避免回避模型身份或虚构平台能力。助手操作栏按消息 `isLast` 与对话收尾态显示，不再误用并不存在的 `msg.status=finished` 把栏吞掉。对外人设从代码内基础 `PROMPT` 下沉到编辑页「系统提示词」（`BaseContext.system_prompt`，默认中文人设，旧英文 `You are a helpful assistant.` 会在默认智能助手启动时迁移），内部执行约束仍由平台追加；上下文摘要触发阈值默认从 100K 调到 900K，以用上 Qwen3.7-Plus 约 1M 窗口并给输出留余量。默认智能助手迁移逻辑改为函数内导入 context 常量，避免 `agent_repository` 模块顶层导入 `yuxi.agents` 触发循环导入导致 API 无法启动。对话支持从指定消息分叉出新线程（原对话保留），并把用户 Memory 从预留开关接到运行：开启后记忆文本注入系统提示。编辑并重发改为先按该消息之前的历史分叉再发送，原对话不动。输入区可开临时会话：不进最近列表/搜索，也不注入 Memory。
...
- 新增用户级配置：保留现有全局配置链路不变，新增 `user_config` 表、`UserConfigSchema` 与无缓存的 `UserConfig` PostgreSQL 读取/保存入口；新增 `/api/user/config`，所有登录用户可读写自己的配置。`enable_memory` 开启后会把 `memory_text` 注入智能体系统提示，设置页可编辑记忆内容；关闭则不注入，已保存文本仍保留。
