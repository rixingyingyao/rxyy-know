...
- 优化智能体对话为 ChatGPT 式连续阅读与操作体验：用户消息改用中性气泡并增加编辑重发、复制操作，助手消息统一展示模型、赞踩、复制、重新生成与来源入口，长对话离开底部后提供回到底部按钮，保留原有流式停止、自动增高输入、附件和 @ 提及能力；基础 Agent Prompt 不再写死“语析”身份，而是注入本次运行真实 `model_spec` 及实际解析出的工具、知识库、Skills 和子智能体，询问模型或能力时必须据此如实回答，避免回避模型身份或虚构平台能力。助手操作栏按消息 `isLast` 与对话收尾态显示，不再误用并不存在的 `msg.status=finished` 把栏吞掉。
