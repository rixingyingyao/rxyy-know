...
# 凭据一律来自环境变量；没配就现场随机生成并在结束时打印一次。
# 不要在这里写死任何默认口令——本文件是公开仓库的一部分，写死等于把口令公开。
SUPERADMIN_UID = os.getenv("YUXI_SUPER_ADMIN_UID", "admin")
SUPERADMIN_NAME = os.getenv("YUXI_SUPER_ADMIN_NAME", "超级管理员")
SUPERADMIN_PHONE_NUMBER = os.getenv("YUXI_SUPER_ADMIN_PHONE") or None
SUPERADMIN_PASSWORD = os.getenv("YUXI_SUPER_ADMIN_PASSWORD") or secrets.token_urlsafe(12)
DEFAULT_USER_PASSWORD = os.getenv("YUXI_DEFAULT_USER_PASSWORD") or secrets.token_urlsafe(12)
...
def main() -> int:
    ...
    print(f"超级管理员密码：{SUPERADMIN_PASSWORD}")
    print(f"部门管理员和普通用户默认密码：{DEFAULT_USER_PASSWORD}")
    print("以上口令只在本次输出，请立即保存；如需固定值请预先设置 "
          "YUXI_SUPER_ADMIN_PASSWORD / YUXI_DEFAULT_USER_PASSWORD 环境变量。")
    return 0
