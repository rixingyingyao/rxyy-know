...
class MilvusGraphService:
    ...
    # rxyy-know 扩展：向量层批量 flush 的 chunk 数。逐 chunk 调 embedding API + Milvus 去重会把
    # 写入吞吐拖到分钟级（实测 5185 chunk 大库 45s/chunk），合批后 API 往返摊薄 ~20x。
    VECTOR_FLUSH_CHUNKS = 20

    def _ensure_graph_indexes(self) -> None:
        """Neo4j MERGE 依赖的属性索引。无索引时 Entity MERGE / relation MATCH 是全标签扫描，
        写入耗时随图规模线性恶化（rxyy-know 扩展，幂等）。"""
        statements = [
            "CREATE INDEX yuxi_entity_merge_idx IF NOT EXISTS "
            "FOR (n:Entity) ON (n.kb_id, n.normalized_name, n.label)",
            "CREATE INDEX yuxi_chunk_id_idx IF NOT EXISTS FOR (n:Chunk) ON (n.chunk_id)",
        ]
    ...
    async def build_pending_chunks(self, kb_id: str, *, batch_size: int, context=None) -> dict[str, Any]:
        ...
        await asyncio.to_thread(self._ensure_graph_indexes)
        ...
        # rxyy-know 扩展：运行级已入向量库的 ID 集合。新闻类语料实体高频复现（深圳/华为...），
        # 命中集合的记录直接跳过 Milvus 去重查询与 embedding，重复实体零成本。
        known_vector_ids: set[str] = set()
        # 待 flush 缓冲：[(chunk, entity_records, triple_records)]，Neo4j/PG 已写完，
        # 向量层与 mark_graph_indexed 延迟到批量 flush（崩溃时 chunk 仍 pending，重跑幂等）。
        vector_buffer: list[tuple[Any, list[dict[str, Any]], list[dict[str, Any]]]] = []

        async def flush_vector_buffer() -> None:
            """合并缓冲内所有 chunk 的实体/三元组：一次去重 + 一次批量 embedding + 一次插入。"""
            nonlocal processed
            if not vector_buffer:
                return
            pending_batch = list(vector_buffer)
            vector_buffer.clear()

            merged_entities: dict[str, dict[str, Any]] = {}
            merged_triples: dict[str, dict[str, Any]] = {}
            for _, entities, triples in pending_batch:
                for entity in entities:
                    entity_id = entity["entity_id"]
                    if entity_id not in known_vector_ids:
                        merged_entities.setdefault(entity_id, entity)
                for triple in triples:
                    triple_id = triple["triple_id"]
                    if triple_id not in known_vector_ids:
                        merged_triples.setdefault(triple_id, triple)

            await self.graph_vector_store.insert_missing_graph_records(
                kb_id=kb_id,
                embedding_model_spec=kb.embedding_model_spec,
                entities=list(merged_entities.values()),
                triples=list(merged_triples.values()),
            )
            known_vector_ids.update(merged_entities.keys())
            known_vector_ids.update(merged_triples.keys())

            for chunk, entities, _ in pending_batch:
        ...
        while True:
                ...
                while True:
                    ...
                    try:
                        ...
                        async with write_lock:
                            ...
                            vector_buffer.append((chunk, entities, triples))
                            if len(vector_buffer) >= self.VECTOR_FLUSH_CHUNKS:
                                await flush_vector_buffer()
            ...
            # 每轮 batch 结束清空缓冲，保证 list_graph_pending 下一轮不重复取到未标记的 chunk
            async with write_lock:
                await flush_vector_buffer()
