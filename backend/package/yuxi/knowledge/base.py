...
class KnowledgeBase(ABC):
    ...
    async def parse_file(self, kb_id: str, file_id: str, operator_id: str | None = None) -> dict:
        ...
        try:
            ...
            # 多模态文件已有完整预处理缓存（打标链路产出）时直接复用，
            # 避免 Omni/ASR/抽帧全套重跑产生双倍 API 费用
            markdown_content = self._get_reusable_preprocessed_text(file_meta)
            if markdown_content is None:
                ...
                logger.info(f"Reusing preprocessed cache for file {file_id}, skipping multimodal processing")
            ...
            # 多模态解析产出的预处理结果写回文件记录，供打标链路缓存复用（入库→打标方向）
            media_cache = params.pop("_media_preprocess", None)
            if media_cache and media_cache.get("text"):
                stored_params = dict(claimed_record.processing_params or {})
                stored_params["preprocessed"] = media_cache
                update_data["processing_params"] = stored_params
                file_meta["processing_params"] = stored_params
    ...
    # 达到该长度说明缓存可能被截断，不能用作入库内容
    _PREPROCESSED_TEXT_LIMIT = 10000

    @staticmethod
    def _get_reusable_preprocessed_text(file_meta: dict) -> str | None:
        """
        多模态文件（音频/视频/图片）的打标预处理缓存若完整可用，返回缓存文本。

        Returns:
            可直接用作入库 markdown 的文本；None 表示需正常解析
        """
        from pathlib import Path as _Path

        from yuxi.knowledge.parser.unified import AUDIO_EXTENSIONS, IMAGE_EXTENSIONS, VIDEO_EXTENSIONS

        cached = (file_meta.get("processing_params") or {}).get("preprocessed") or {}
        text = cached.get("text")
        if not text or not isinstance(text, str):
            return None
        if len(text) >= KnowledgeBase._PREPROCESSED_TEXT_LIMIT:
            return None  # 可能被截断，重新解析保证入库内容完整

        name = file_meta.get("filename") or file_meta.get("path") or ""
        suffix = _Path(name.split("?")[0]).suffix.lower()
        if suffix not in (*AUDIO_EXTENSIONS, *VIDEO_EXTENSIONS, *IMAGE_EXTENSIONS):
            return None  # 普通文档解析便宜且解析器更优，不走缓存
