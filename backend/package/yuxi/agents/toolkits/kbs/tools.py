...
async def _build_query_output(target_kb_id: str, result: Any) -> Any:
    ...
    return await _enrich_with_file_tags(output)


async def _enrich_with_file_tags(output: Any) -> Any:
    """rxyy-know 扩展：为检索结果注入命中文件的媒资标签，帮助 LLM 理解内容分类。

    标签存于 KnowledgeFile.processing_params.tags（媒资打标结果），
    仅在输出 dict 追加 file_tags 字段，不改动 chunk 内容。
    """
    if not isinstance(output, dict) or not isinstance(output.get("results"), list):
        return output

    file_ids = list({r.get("file_id") for r in output["results"] if isinstance(r, dict) and r.get("file_id")})
    if not file_ids:
        return output
    ...
    try:
        ...
        for record in records:
            tags = (record.processing_params or {}).get("tags") or []
            tag_names = [t["tag_name"] for t in tags if isinstance(t, dict) and t.get("tag_name")]
            if tag_names:
                file_tags[record.file_id] = tag_names[:8]
        if file_tags:
            output["file_tags"] = file_tags
    except Exception as exc:  # noqa: BLE001
        logger.debug(f"注入文件标签失败（忽略）: {exc}")
    return output
