...
class OtherEmbedding(BaseEmbeddingModel):
    ...
    @staticmethod
    def _should_split(response, message) -> bool:
        """百炼 embeddings 单请求条数/token 超限会返回 400。批量含多条时二分重试可绕过。

        单条仍 400 说明是该条本身超 token 上限，无法再拆 -> 交给常规错误处理。
        """
        status_code = getattr(response, "status_code", None)
        if status_code != 400:
            return False
    ...
    def encode(self, message: list[str] | str) -> list[list[float]]:
        ...
        while True:
            try:
                ...
                if self._should_split(resp, message):
                    mid = len(message) // 2
                    logger.warning(
                        f"Embedding 400 on {len(message)} inputs; splitting into {mid}+{len(message) - mid} and retrying"
                    )
                    return self.encode(message[:mid]) + self.encode(message[mid:])
        ...
        async with httpx.AsyncClient() as client:
            ...
            while True:
                try:
                    ...
                    if self._should_split(e.response, message):
                        mid = len(message) // 2
                        logger.warning(
                            f"Embedding 400 on {len(message)} inputs; splitting into "
                            f"{mid}+{len(message) - mid} and retrying"
                        )
                        first = await self.aencode(message[:mid])
                        second = await self.aencode(message[mid:])
                        return first + second
                    retry = self._prepare_retry(
                        message,
                        retry_index=retry_index,
