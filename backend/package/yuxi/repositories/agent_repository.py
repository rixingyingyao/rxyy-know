...
效率硬约束（防止研究过程无限展开）：
- 调研分「轮」：首轮把所有子问题一次性并行派发；之后**最多再补 1 轮**调研，仅针对首轮暴露的关键证据缺口，不新开角度。
- 核查合并派发：把需要核验的关键论断攒成一批，**一次性**派给核查子智能体，禁止逐条多次派发。
- 两轮调研 + 一轮核查后必须收敛成稿；残余缺口在报告中标注「证据不足」，不再追加检索。
...
class AgentRepository:
    ...
    async def ensure_preset_agents(self, *, created_by: str | None = None) -> None:
        """落库 rxyy-know 扩展内置智能体：主题图谱助手 + 媒资 AI 助手。"""
        from yuxi.agents.preset_prompts import (
            MEDIA_AGENT_DESCRIPTION,
            MEDIA_AGENT_NAME,
            MEDIA_AGENT_SLUG,
            MEDIA_AGENT_SYSTEM_PROMPT,
            SYSTEM_PROMPT as TOPIC_GRAPH_SYSTEM_PROMPT,
            TOPIC_GRAPH_AGENT_DESCRIPTION,
            TOPIC_GRAPH_AGENT_NAME,
            TOPIC_GRAPH_AGENT_SLUG,
        )

        await self._ensure_builtin_agent(
            slug=TOPIC_GRAPH_AGENT_SLUG,
            backend_id=DEFAULT_AGENT_BACKEND_ID,
            name=TOPIC_GRAPH_AGENT_NAME,
            description=TOPIC_GRAPH_AGENT_DESCRIPTION,
            config_context={
                "system_prompt": TOPIC_GRAPH_SYSTEM_PROMPT,
                "tools": ["complete_kb_from_web"],
        ...
        await self._ensure_builtin_agent(
            slug=MEDIA_AGENT_SLUG,
            backend_id=DEFAULT_AGENT_BACKEND_ID,
            name=MEDIA_AGENT_NAME,
            description=MEDIA_AGENT_DESCRIPTION,
            config_context={
