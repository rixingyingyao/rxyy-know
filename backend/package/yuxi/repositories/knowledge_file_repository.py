...
from yuxi.storage.postgres.models_knowledge import JSON_VALUE, KnowledgeFile
...
class KnowledgeFileRepository:
    ...
    async def reset_interrupted_states(self) -> dict[str, int]:
        """服务重启时复位遗留的中间态文件，避免成为无任务承载的孤儿。

        parsing  -> uploaded（回到待解析，可被重新解析）
        indexing -> parsed  （回到待入库，可被重新入库）

        背景：Tasker 内存队列在进程重启后丢失，正在 parsing/indexing 的文件
        会永久停在中间态（无活动任务推进）。启动时统一回退到上一个可续跑状态，
        配合 index-pending / parse-pending 即可自动补齐，不再需要人工介入。
        """
        now = utc_now_naive()
        resets = {"parsing": 0, "indexing": 0}
        async with pg_manager.get_async_session_context() as session:
            for stuck, fallback in (("parsing", "uploaded"), ("indexing", "parsed")):
                result = await session.execute(
                    update(KnowledgeFile)
                    .where(KnowledgeFile.status == stuck)
                    .values(
                        status=fallback,
                        updated_at=now,
                        error_message="服务重启复位：中间态回退，等待重新处理",
                    )
                )
                resets[stuck] = int(result.rowcount or 0)
    ...
    async def _list_directory_documents(
        ...
        *,
        ...
        real_select = select(
            ...
            KnowledgeFile.processing_params.label("processing_params"),
        ...
        virtual_select = (
            select(
                ...
                cast(literal(None), JSON_VALUE).label("processing_params"),
