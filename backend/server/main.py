...
# SL 生产部署适配：外层 openresty 反代的 add_header 缺少 always 修饰符，
# 仅在 2xx/3xx 添加 Access-Control-Allow-Origin，4xx/5xx 响应会被浏览器按 CORS 错误拦截。
# 该兜底仅在错误状态码时补 ACAO，避免与反代产生双重 CORS 头；
# 流式响应加 X-Accel-Buffering: no 禁用反代缓冲，否则 SSE/chunked 会被反代攒到结束才返回。
@app.middleware("http")
async def supplement_cors_on_error(request: Request, call_next):
    response = await call_next(request)
    if response.status_code >= 400:
