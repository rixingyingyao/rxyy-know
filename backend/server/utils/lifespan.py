...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        async with pg_manager.get_async_session_context() as session:
            ...
            await repository.ensure_preset_agents()
    except Exception as e:
    ...
    if os.environ.get("LITE_MODE", "").lower() in ("true", "1"):
        ...
        # 复位重启遗留的 parsing/indexing 中间态，避免文件成为无任务孤儿
        try:
            from yuxi.repositories.knowledge_file_repository import KnowledgeFileRepository

            resets = await KnowledgeFileRepository().reset_interrupted_states()
            if resets.get("parsing") or resets.get("indexing"):
                logger.info(
                    "复位重启遗留中间态文件: parsing->uploaded={}, indexing->parsed={}".format(
                        resets.get("parsing", 0), resets.get("indexing", 0)
                    )
                )
        except Exception as e:
            logger.error(f"Failed to reset interrupted knowledge file states: {e}")
