...
from server.utils.kb_permission import (
    ensure_kb_manageable,
    ensure_kb_quota,
    force_private_share_config,
    get_kb_manager,
    get_kb_reader,
    user_info,
)
...
# 批量解析/入库并发上限（OCR/多模态预处理是 IO+外部 API 密集型，rxyy-know 扩展并发改造）
KB_BATCH_CONCURRENCY = max(1, int(os.getenv("KB_BATCH_CONCURRENCY", "3")))
...
def _attach_can_manage(db_info: dict, current_user: User) -> dict:
    """给知识库信息补 can_manage，前端据此决定显不显示编辑/删除/上传按钮。"""
    db_info["can_manage"] = knowledge_base.is_database_manageable(
...
async def create_database(
    ...
    await ensure_kb_quota(current_user)
    share_config = force_private_share_config(current_user, share_config)
    try:
        # 先检查名称是否已存在
        ...
        return _attach_can_manage(database_info, current_user)
...
async def get_mindmap_databases(current_user: User = Depends(get_required_user)):
...
async def get_database_mindmap_files(kb_id: str, current_user: User = Depends(get_kb_reader)):
    """获取指定知识库的所有文件列表。"""
    try:
...
async def generate_mindmap(
    ...
    current_user: User = Depends(get_kb_manager),
):
...
async def get_database_mindmap(kb_id: str, current_user: User = Depends(get_kb_reader)):
    """获取知识库关联的思维导图。"""
    try:
...
async def get_mindmap_diff_route(kb_id: str, current_user: User = Depends(get_kb_reader)):
    """检测思维导图与知识库文件的变更差异。"""
    try:
...
async def get_database_info(
    ...
    return _attach_can_manage(database, current_user)


@knowledge.post("/databases/{kb_id}/stats/repair")
async def repair_database_stats(kb_id: str, current_user: User = Depends(get_kb_manager)):
...
async def delete_database(kb_id: str, current_user: User = Depends(get_kb_manager)):
    """删除知识库"""
...
async def get_graph_build_status(kb_id: str, current_user: User = Depends(get_kb_reader)):
    try:
...
async def add_documents(
    kb_id: str, items: list[str] = Body(...), params: dict = Body(...), current_user: User = Depends(get_kb_manager)
):
    """添加文档到知识库（上传 -> 解析 -> 可选入库）"""
...
async def _run_parse_file_ids(
    *,
    ...
    sem = asyncio.Semaphore(KB_BATCH_CONCURRENCY)
    ...
    async def parse_one(file_id: str) -> None:
        ...
        async with sem:
            await context.raise_if_cancelled()
            try:
                # rxyy-know 扩展：解析前持久化参数覆盖（如 ocr_engine / ocr_fallback）
                if params:
                ...
                await context.set_progress(5.0 + (done / total) * 90.0, f"已解析 {done}/{total} 个文档")

    if file_ids:
        await asyncio.gather(*(parse_one(fid) for fid in file_ids))
...
async def _run_index_file_ids(
    *,
    ...
    index_done = 0
    index_sem = asyncio.Semaphore(KB_BATCH_CONCURRENCY)

    async def index_one(file_id: str) -> None:
        nonlocal index_done
        async with index_sem:
            ...
            try:
                ...
                await context.set_progress(
                    5.0 + (index_done / total) * 90.0, f"已入库 {index_done}/{total} 个文档"
                )

    pending_ids = [fid for fid in file_ids if fid not in param_update_failed]
    if pending_ids:
        await asyncio.gather(*(index_one(fid) for fid in pending_ids))
...
async def parse_documents(
    kb_id: str, payload: list[str] | dict = Body(...), current_user: User = Depends(get_kb_manager)
):
    """手动触发文档解析。

    请求体兼容两种格式（rxyy-know 扩展）：
    - 裸列表 ``["file_id", ...]``（上游原格式）
    - 对象 ``{"file_ids": [...], "params": {...}}``，params 支持覆盖解析参数（如 ocr_engine / ocr_fallback）
    """
    if isinstance(payload, dict):
        file_ids = payload.get("file_ids") or []
        params = payload.get("params") or {}
    else:
        file_ids, params = payload, {}
...
async def parse_pending_documents(kb_id: str, current_user: User = Depends(get_kb_manager)):
    """按状态手动触发全部待解析文档解析。"""
...
async def get_document_info(kb_id: str, doc_id: str, current_user: User = Depends(get_kb_reader)):
...
async def get_document_basic_info(kb_id: str, doc_id: str, current_user: User = Depends(get_kb_reader)):
    """获取文档基本信息（仅元数据）"""
...
async def get_document_content(kb_id: str, doc_id: str, current_user: User = Depends(get_kb_reader)):
...
async def batch_delete_documents(
    kb_id: str, file_ids: list[str] = Body(...), current_user: User = Depends(get_kb_manager)
):
    """批量删除文档或文件夹"""
...
async def delete_document(kb_id: str, doc_id: str, current_user: User = Depends(get_kb_manager)):
    """删除文档或文件夹"""
...
async def download_document(kb_id: str, doc_id: str, current_user: User = Depends(get_kb_reader)):
    """下载原始文件"""
...
@knowledge.get("/databases/{kb_id}/download-all")
async def download_all_files(kb_id: str, current_user: User = Depends(get_kb_reader)):
    """批量下载知识库所有原始文件（ZIP 打包，rxyy-know 扩展）"""
    import io
    import zipfile

    from yuxi.repositories.knowledge_file_repository import KnowledgeFileRepository

    await _ensure_database_supports_documents(kb_id, "批量下载")
    try:
        database = await knowledge_base.get_database_info(kb_id)
        if database is None:
            raise HTTPException(status_code=404, detail="知识库不存在")
        db_name = database.get("name") or kb_id

        records = await KnowledgeFileRepository().list_by_kb_id(kb_id)
        file_records = [r for r in records if not getattr(r, "is_virtual_folder", False)]
        if not file_records:
            raise HTTPException(status_code=404, detail="知识库中没有文件")
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            for record in file_records:
                file_path = record.minio_url or record.path or ""
                ...
                if filename in used_names:
                    filename = f"{record.file_id}_{filename}"
                try:
                    if is_minio_url(file_path):
                        bucket_name, object_name = parse_minio_url(file_path)
                        data = await minio_client.adownload_file(bucket_name, object_name)
                        zf.writestr(filename, data)
                        used_names.add(filename)
                        added += 1
                    else:
                        logger.warning(f"Skipping file {record.file_id}: no downloadable source ({file_path})")
                except Exception as e:
                    logger.warning(f"Failed to add file {record.file_id} to zip: {e}")

        if not added:
            raise HTTPException(status_code=404, detail="没有可下载的原始文件")

        buf.seek(0)
        encoded_zip_name = quote(f"{db_name}.zip".encode())
        ...
        logger.error(f"批量下载失败: {e}, {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"批量下载失败: {e}")
...
async def query_knowledge_base(
    kb_id: str, query: str = Body(...), meta: dict = Body(...), current_user: User = Depends(get_kb_reader)
):
    """查询知识库"""
...
async def update_knowledge_base_query_params(
    kb_id: str, params: dict = Body(...), current_user: User = Depends(get_kb_manager)
):
    """更新知识库查询参数配置"""
    try:
        # 获取知识库实例
...
async def get_knowledge_base_query_params(kb_id: str, current_user: User = Depends(get_kb_reader)):
    """获取知识库类型特定的查询参数"""
    try:
        # 获取知识库实例
        kb_instance = await knowledge_base._get_kb_for_database(kb_id)
...
async def get_sample_questions(kb_id: str, current_user: User = Depends(get_kb_reader)):
    """获取知识库的测试问题。"""
    try:
...
async def upload_file(
    ...
    try:
        file_bytes = await read_upload_with_limit(
            ...
            too_large_message=f"文件过大，当前仅支持 {MAX_UPLOAD_SIZE_BYTES // (1024 * 1024)} MB 以内的文件",
        )
        validate_ooxml_bytes(filename, file_bytes)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def mark_it_down(file: UploadFile = File(...), current_user: User = Depends(get_required_user)):
    """调用统一 Parser 将文件解析为 markdown（登录用户可用，不落库）"""
    import tempfile
