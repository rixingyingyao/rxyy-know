<template>
  ...
  >
    ...
    <div v-else-if="message.type === 'ai'" class="assistant-message">
      ...
      <!-- 消息内容（graph-viz 代码块单独提取渲染） -->
      <MarkdownPreview
        v-if="parsedData.contentWithoutGraphViz"
        :key="message.id + '-md'"
        :content="parsedData.contentWithoutGraphViz"
        code-copy
        class="message-md"
      />

      <!-- graph-viz 图谱块：识别 ```graph-viz JSON ``` 并用 ECharts 渲染 -->
      <GraphVizRenderer
        v-for="(g, idx) in parsedData.graphVizBlocks"
        :key="message.id + '-gv-' + idx"
        :data="g"
        @ask-followup="(q) => emit('askFollowup', q)"
      />

      <div
        v-if="
          !parsedData.contentWithoutGraphViz &&
          !parsedData.graphVizBlocks.length &&
...
function extractGraphVizBlocks(text) {
  if (!text) return { graphVizBlocks: [], contentWithoutGraphViz: text || '' }

  const regex = /```graph-viz\s*([\s\S]+?)```/g
  const blocks = []
  let matched
  while ((matched = regex.exec(text)) !== null) {
    ...
    try {
      ...
      console.warn('[GraphViz] JSON parse failed:', e, raw.slice(0, 200))
    }
  }
  const contentWithoutGraphViz = text.replace(regex, '').trim()
  return { graphVizBlocks: blocks, contentWithoutGraphViz }
}

const parsedData = computed(() => {
  const { content, reasoningContent } = MessageProcessor.parseAssistantMessageBody(props.message)
  const { graphVizBlocks, contentWithoutGraphViz } = extractGraphVizBlocks(content)
  return {
    content,
