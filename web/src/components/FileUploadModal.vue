...
const ocrEngineOptions = [
  ...
  },
  {
    value: 'qwen_ocr',
    label: 'Qwen-OCR (百炼)',
    description: '阿里百炼 qwen3.5-ocr 云端文字提取'
  },
  ...
  if (uploadMode.value === 'workspace') {
    ...
    try {
      ...
      for (const item of importedItems) {
        ...
        // 图片未启用 OCR 时后端走 VL 画面描述入库（不再强制要求 OCR），仅作提示
        const ext = filePath.substring(filePath.lastIndexOf('.')).toLowerCase()
        if (imageExtensions.includes(ext) && !isOcrEnabled.value && !imageVlNoticeShown) {
          imageVlNoticeShown = true
          message.info({
            content:
              '图片未启用 OCR，将使用 VL 视觉描述入库；如图片含大量文字建议启用 OCR 以获得更好检索效果。',
            duration: 6
          })
        }
      }
