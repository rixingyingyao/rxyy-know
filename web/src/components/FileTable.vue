    ...
    >
            ...
            <template #overlay>
              <div class="overflow-menu-panel" @click.stop>
                <div class="overflow-actions">
                  ...
                  <div
                    class="overflow-action-item"
                    :class="{ 'is-loading': downloadingAll }"
      ...
      <template #before-table>
        <div class="batch-actions" v-if="isSelectionMode">
          ...
          <div class="batch-parse-ocr" v-if="canBatchParse">
            <a-select
              v-model:value="batchOcrOverride"
              :options="batchOcrOptions"
              size="small"
              class="batch-ocr-select"
              placeholder="OCR (保持原设置)"
              allow-clear
            />
            <a-checkbox v-model:checked="batchOcrFallback" class="batch-ocr-fallback">
              失败自动尝试其他 OCR
            </a-checkbox>
          </div>
      ...
      <template #cell-tags="{ row }">
        <div class="file-tags-cell" @click.stop>
          <template v-if="!row.is_folder && row.tags && row.tags.length">
              ...
              <template #content>
                <div style="max-width: 400px">
                  <a-tag
                    ...
                    :key="tag.tag_id"
                    :color="tag.source === 'rules' ? 'green' : tag.is_suggested ? 'orange' : undefined"
                    style="margin: 2px"
                  >
                    {{ tag.tag_name }}
                    <span style="opacity: 0.6; font-size: 11px">
                      {{ ((tag.confidence || 0) * 100).toFixed(0) }}%
                    </span>
                  </a-tag>
                </div>
              </template>
              <span style="cursor: pointer">
                <a-tag v-for="tag in row.tags.slice(0, 2)" :key="tag.tag_id" style="margin-right: 2px">
                  {{ tag.l2_display ? tag.l2_display.l2_name || tag.tag_name : tag.tag_name }}
                </a-tag>
                <span v-if="row.tags.length > 2" style="font-size: 12px; opacity: 0.65">
                  +{{ row.tags.length - 2 }}
                </span>
              </span>
            </a-popover>
          </template>
          <span v-else-if="!row.is_folder" style="opacity: 0.35">-</span>
        </div>
      </template>
...
// rxyy-know 扩展：批量解析 OCR 覆盖（null 表示保持文件原本的 processing_params）
const batchOcrOverride = ref(null)
const batchOcrFallback = ref(false)
// mineru_ocr / pp_structure_v3_ocr 依赖本地 mineru/paddlex 服务，当前环境未部署，暂不展示（起服务后可加回）
const batchOcrOptions = [
  { value: 'disable', label: '不启用 OCR' },
  { value: 'rapid_ocr', label: 'RapidOCR (ONNX)' },
  { value: 'qwen_ocr', label: 'Qwen-OCR (百炼)' },
  { value: 'mineru_official', label: 'MinerU 官方云' },
...
// rxyy-know 扩展：打包下载知识库全部原始文件
const downloadingAll = ref(false)
const handleDownloadAll = async () => {
  if (downloadingAll.value) return
  const dbId = store.kbId
  if (!dbId) {
  ...
  try {
    const response = await documentApi.downloadAll(dbId)
    const contentDisposition = response.headers.get('content-disposition')
    let filename = `${store.database?.name || 'knowledge-base'}.zip`
    ...
    message.error('批量下载失败: ' + (e.message || '未知错误'))
  } finally {
...
// rxyy-know 扩展：批量打标（选中文件创建打标任务，进度在媒资打标页查看）
const batchTagging = ref(false)
const handleBatchTag = async () => {
  const fileIds = selectedRowKeys.value.filter((id) => !String(id).startsWith('__virtual_folder__'))
  if (!fileIds.length) return
  batchTagging.value = true
  try {
    const res = await taggingApi.batchTag(store.kbId, fileIds)
    message.success(`已创建 ${res.count} 个打标任务，可在「媒资打标」页跟踪审核`)
    selectedRowKeys.value = []
  } catch (e) {
    message.error('批量打标失败: ' + (e.message || '未知错误'))
  } finally {
...
const handleBatchParse = async () => {
  ...
  // rxyy-know 扩展：可选覆盖 OCR 引擎 / 失败自动 fallback（持久化进 processing_params）
  const params = {}
  if (batchOcrOverride.value) params.ocr_engine = batchOcrOverride.value
  if (batchOcrFallback.value) params.ocr_fallback = true

  await store.parseFiles(validKeys, params)
  selectedRowKeys.value = []
}
